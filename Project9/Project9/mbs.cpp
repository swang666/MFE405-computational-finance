...
double MBS(double WAC, int T, double PV0, double r0, double kappa, double r_bar, double sigma, int64_t seed) {
	...
	for (int i = 0; i < sim_num; ++i) {
		...
		double* uni = getLGM(N, seed + 1000 * i);
		double* rand = box_muller(uni, N);
		...
		for (int j = 0; j < N; ++j) {
			...
			rt_1_10[j] = -1 / 10 * trapzoid_method(j, min(j + 120,N), rt, dt);
			...
			TPP_t[j] = PVt_1[j] * r * (1 / (1 - pow((1 + r), (-N + j))) - 1) + (PVt_1[j] - PVt_1[j] * r*(1 / (1 - pow((1 + r), (-N + j))) - 1))*(1 - pow(1 - CPR_t[j], 1 / 12));
			IP_t[j] = PVt_1[j] * r;
