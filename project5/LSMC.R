...
LSMC_eu = function (N, t, Kt, S0, r, sigma, num){
  paths = matrix(rep(0, len = num *(N+1)), nrow = num)
  rand = matrix(rnorm(num*N), nrow = num)
  dt = t/N
  paths[, 1] = S0
  ...
  strike_date = Kt/dt + 1
  K = mean(paths[, strike_date])
  payoff = apply(cbind(K - paths[, N+1], zeros), 1, max)
  return(mean(exp(-r*t)*payoff))
}

LSMC_us = function (N, t, Kt, S0, r, sigma, num, k, FUN){
  paths = matrix(rep(0, len = num *(N+1)), nrow = num)
  rand = matrix(rnorm(num*N), nrow = num)
  CF = matrix(rep(0, len = num *(N)), nrow = num)
  dt = t/N
  ...
  return(mean(CF[,strike_date:N]%*% D))
}
